...
VOID SD_StorageParse(FileListTypeDef *file_list)
{
  ...
  do
  {
    ...
    file_exist = fx_directory_next_full_entry_find(&sdio_disk, object_name, UX_NULL, &object_size, UX_NULL,
                                                   UX_NULL, UX_NULL, UX_NULL, UX_NULL, UX_NULL);
    ...
    if ((strstr(object_name, "wav")) || (strstr(object_name, "WAV")))
    {
      ...
      USBH_UsrLog((char *)file_list->file[file_list->ptr].name);
