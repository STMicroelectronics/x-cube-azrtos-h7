...
VOID audio_playback_thread_entry(ULONG thread_input)
{
  ...
  SHORT min, max;
  ULONG duration;
  CHAR str[FX_MAX_LONG_NAME_LEN + 20];
        ...
        (audio_playback->ux_host_class_audio_state == UX_HOST_CLASS_INSTANCE_LIVE))
    {
      ...
      switch(audio_demo_playback_state)
      {
        ...
        case AUDIO_DEMO_PLAYBACK_START:
          ...
          /* Get audio Wav file infomation from header  */
          AUDIO_GetFileInfo(file_index, &FileList, &WavPlayInfo);

          /* Print wav name */
          sprintf((char *)str, "Playing file (%d/%d): %s", file_index+1, FileList.ptr, (char *)FileList.file[file_index].name);
          USBH_UsrLog(str);

          /* Print wav sample rate */
          sprintf((char *)str, "Sample rate : %lu Hz", WavPlayInfo.SampleRate);
          USBH_UsrLog(str);

          /* Print wav channel number */
          sprintf((char *)str, "Channels number : %d", WavPlayInfo.NbrChannels);
          USBH_UsrLog(str);

          /* Print wav duration */
          duration = WavPlayInfo.FileSize / WavPlayInfo.ByteRate;
          sprintf((char *)str, "File Size : %lu MB [%02lu:%02lu]", WavPlayInfo.FileSize / 1024, duration / 60, duration % 60);
          USBH_UsrLog(str);
