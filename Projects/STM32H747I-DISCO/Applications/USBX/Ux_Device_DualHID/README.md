...
At the beginning ThreadX call the entry function tx_application_define(), at this stage, all USBx resources are initialized, the HID Class driver is
...
For each 10ms, the application calls the GetKeyData() API to update thekey character and send
the report buffer through the ux_device_class_hid_event_set() API.
