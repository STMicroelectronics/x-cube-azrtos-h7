...
{
    UINT ret = NX_SUCCESS;
    /* USER CODE BEGIN  App_NetXDuo_Inuit */

  /* App memory pointer. */
  UCHAR   *pointer;
  /* Initialize app pointer to memory pointer. */
  ...
  {
    /* Increment pointer with  NX_PACKET_POOL_SIZE. */
    pointer = pointer + NX_PACKET_POOL_SIZE;
  }

   /* Create the server packet pool. */
   ret = nx_packet_pool_create(&WebServerPool, "HTTP Server Packet Pool", SERVER_PACKET_SIZE, pointer, SERVER_POOL_SIZE);
  ...
  {
    /* Increment pointer with  SERVER_POOL_SIZE. */
    pointer = pointer + SERVER_POOL_SIZE;
  }

  /* Create an IP instance by linking the nx_driver_St32h7xx driver */
  ret = nx_ip_create(&EthIP, "NetX IP Instance 0", NULL_IP_ADDRESS, NULL_IP_ADDRESS, &EthPool, nx_driver_stm32h7xx,
  ...
  {
    /* Increment pointer with  MEMORY_SIZE. */
  ...
  {
    /* Increment pointer with  ARP_CACHE_SIZE. */
  ...
  {
    pointer = pointer + SERVER_STACK;
  }

  /* Create the main thread. */
  ret = tx_thread_create(&Main_thread, "Main_thread", nx_server_thread_entry, ENTRY_INPUT, pointer, WEB_APP_SIZE,
                    DEFAULT_PRIORITY, THREAD_PREEMPT_THRESHOLD, TX_NO_TIME_SLICE, TX_AUTO_START);
  if(ret != TX_SUCCESS)
  {
