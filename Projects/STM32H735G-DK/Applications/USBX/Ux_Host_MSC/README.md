...
##  Ux_Host_MSC application description
...
The application is designed to behave as an USB MSC Host able to operate with an USB flash disk using the Bulk Only Transfer (BOT) and Small Computer System Interface (SCSI) transparent commands combined with a file system AzureRTOS FILEX.
...
The application creates one thread : 
  - usbx_app_thread_entry (Priority : 20; Preemption threshold : 20) used to initialize USB OTG HAL HCD driver, start the Host and proceed to file operations once the device is properly enumerated.
