...
INT lx_stm32_ospi_write(UINT instance, ULONG *address, ULONG *buffer, ULONG words)
{
  ...
  do
  {
    ...
    if (HAL_OSPI_Transmit(&hospi1, (uint8_t*)data_buffer, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    {
      return 1;
    }
...
int status = 0;
  ...
  /* USER CODE BEGIN OSPI_HAL_CFG_OCTALMODE */
  ...
  else
  {
    ...
    /* Write Configuration register 2 (with Octal I/O SPI protocol) */
    s_command.Instruction     = LX_STM32_OSPI_OCTAL_WRITE_CFG_REG2_CMD;
    s_command.InstructionMode = HAL_OSPI_INSTRUCTION_8_LINES;
    s_command.InstructionSize = HAL_OSPI_INSTRUCTION_16_BITS;
    s_command.Address         = LX_STM32_OSPI_CR2_REG1_ADDR;
    s_command.AddressMode     = HAL_OSPI_ADDRESS_8_LINES;
    ...
    /* Check the configuration has been correctly done */
    s_command.Instruction = LX_STM32_OSPI_READ_CFG_REG2_CMD;
    s_command.Address     = LX_STM32_OSPI_CR2_REG1_ADDR;
    s_command.AddressMode = HAL_OSPI_ADDRESS_1_LINE;
  ...
  /* USER CODE END  OSPI_HAL_CFG_OCTALMODE */
