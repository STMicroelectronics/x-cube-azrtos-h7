                               ...
                               const char *name, whd_mac_t *mac_addr,  whd_interface_t *ifpp)
{
    ...
    if (bsscfgidx < WHD_INTERFACE_MAX)
    {
        ...
        if ( (ifp = (whd_interface_t)WHD_MALLOC(sizeof(struct whd_interface) ) ) != NULL )
        {
                  ...
                  whd_netif_funcs_t *network_ops)
{
    ...
    if ( (whd_drv = (whd_driver_t)WHD_MALLOC(sizeof(struct whd_driver) ) ) != NULL )
    {
